...
final class X {
    ...
    public function get_auth_url() : array {
        ...
        // Callback URL (Must match Twitter Dev Portal exactly)
        ...
        // Save secret temporarily (10 mins) to verify later
    ...
    /**
     * Step 2: Handle the Callback (Exchanges Code -> Access Token)
     */
    public function handle_callback($token, $verifier) : array {
        ...
        if (!$temp_secret) {
            return ['ok' => false, 'error' => 'Session expired or invalid token. Please try connecting again.'];
        }
    ...
    public function post_tweet(int $post_id, string $text) : array {
        ...
        // V2 API expects JSON payload
        $payload = ['text' => $text . "\n\n" . $link];

        // Sign Request (OAuth 1.0a User Context)
        ...
        // Success check for V2
        if (!isset($json['data']['id'])) {
            $err = $json['detail'] ?? ($json['title'] ?? 'Unknown X API Error');
